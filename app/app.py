...
for title, rating in data.items():                     # title = erster Teil, value = „key“
    if title == "key" or title.startswith("FormSubmitter"):
        continue                                      # Hilfsschlüssel überspringen
    if rating is not None and rating != "null":  # Überprüfen, ob ein Rating vorhanden ist
        rating = rating+1                         # nur falls ein Rating vorhanden
